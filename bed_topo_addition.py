...
from scipy import interpolate # need to interpolate bed topo data
...
rows = np.loadtxt('gle_matrices_25rms_10fdim_zsmooth.csv', delimiter = ",")
rows = rows.transpose()
...
#numpy.loadtxt(gle_matrices_75rms_75fdim_zsmooth.csv)
L = 80429.                  # Characteristic domain length, 
                            # (255000-94141)/2 --> (xfine max - xfine min) / 2
...
be = df.Constant(1e-8)         # Bedrock erosion coefficient, high = 1e-8, low = 1e-9
cc = df.Constant(2e-10)        # Fluvial erosion coefficient, high = 2e-10, low = 2e-12
d = df.Constant(500.0)         # Fallout fraction
h_0 = df.Constant(0.1)         # Subglacial cavity depth 

# l_s = df.Constant(2.0)         # Sediment thickness at which bedrock erosion becomes negligible 
# # be = df.Constant(1e-8)         # Bedrock erosion coefficient, 4.8 +/- 2.66 mm/a  (Cowton et al., 2012)
# be = df.Constant(5e-09)         # upper = 7.46, lower =  2.13999
#                                 # double = 2e-08, half = 5e-09
# cc = df.Constant(1e-06)        # Fluvial erosion coefficient, change order to 10-7 or 10-6 (Doyle et al., 2021)
#                                 # double = 4e-06, half = 1e-06
# d = df.Constant(500.0)         # Fallout fraction, 12.3–44.8 m3 m−1 a−1(Knight et al., 2002; see table 4)
#                                 # high magnitude order 1 = 5000.0, low magnitude order 1 = 50.0
# h_0 = df.Constant(0.1)         # Subglacial cavity depth  
...
for i in range(len(rows))[0:10]:
      bed_topo = rows[i]
      trial_ice_name = 'Trial' + str(i)
    
     # Topography
      z = np.zeros(1371) #this is not called for after this line
      N_steps = len(z) 
      dx = (2*L) / N_steps
      x = np.linspace(-L, L, N_steps)
      z_interpolate = interpolate.interp1d(x, z, fill_value="extrapolate") #interpolate bed topo data, on 03/14 changed interp1d(z,x) to (x,z) where x is domain and z is range
     ...
     #plt.show()
      ...
      class Surface(df.UserExpression):
        def eval(self,values,x):
          ...
          #values[0] = z + thklim #insert my data here, concantanates to a different data type here
    
      # Bed elevation
      class Bed(df.UserExpression):
        def eval(self,values,x):
          values[0] = ((zmax - zmin)*np.exp(-(x[0]+L)/(L*0.3)) + zmin - amp*(np.sin(4*np.pi*x[0]/L))) + z_interpolate(x[0])
          #values[0] = z #insert my data here, z data format is the issue, maybe change float64 to float (or vice versa)
          #values[0] = z_interpolate(x[0]) #insert interpolated bed topo
      ...
      #possibly need to define a function space here to behave with interpolating my bed topo
      ...
      B0 = df.interpolate(Bed(),Q_cg) #bed interpolation
      B0_ = df.interpolate(Bed(),Q_dg) #smoothing?
      ...
      ax[3].set_ylabel('Sed. Thk. (m)')
      ax[3].set_xlabel('Distance along bed topography (m)')
      ...
      while t<t_end:
          ...
          try: # If the solvers don't converge, reduce the time step and try again.
            ...
            # print('solving mass')
              ...
              ## Solve for ice volume, surface geometry, and grounding line
              ice_vol = df.assemble(H*df.dx)
              surface_geometry = df.project(S).compute_vertex_values()
              ...
              bed = B0.compute_vertex_values()  #changed B0 to z (my bed topo), problem with function spaces (Q_cg, E_cg) and functions (BO, BO_)
              ...
              # 1029/917 (some_threshold): (density of ocean water / density of glacier ice) 
              ...
              ice_vol_data.append(ice_vol)
              surface_geometry_data.append(surface_geometry)
    
              t+=dt_float # += : syntax represents add amount and build array
              counter+=1
          except RuntimeError:
              dt_float/=2.
              dt.assign(dt_float)
      ...
      #pickle.dump((tdata,Hdata,hdata,Bdata,Hgldata,ugldata,aardata,usdata,ubdata,grdata,gldata,massdata,Qgldata,adotplusdata,adotminusdata,edotdata,ddotdata,bdotdata),open('results/temperate_experiment','wb'))
    
      #pickle.dump((ice_vol, surface_geometry),open('results/temperate_experiment','wb'))
     
      ice_vol_dict[trial_ice_name] = ice_vol_data
      ## first attempt to save as DF, Trial0 = 129 but Trial1 = 100 --> trim for now
     
      plt.close()
     
      #np.savetxt("results/surface_geometry_data.txt", surface_geometry_data)
      #np.savetxt("results/ice_vol_data.txt", ice_vol_data)
    
      #output_vars=gldata
      #export_storage.append(gldata)
    # export_storage = gldata
    
for k in ice_vol_dict.keys():
    ice_vol_dict[k] = ice_vol_dict[k][0:1000] ## trim if needed for consistent length
df_ice_vol = pd.DataFrame.from_dict(ice_vol_dict)
#transpose dataframe
df_ice_vol.to_csv('high_sed_25r_10f.csv', header=True)
